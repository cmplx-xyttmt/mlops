...
class HPTuneCallback(TrainerCallback):
    """
    A custom callback class that reports a metric to hypertuner at the end of each epoch
    """
...
def train(args, model, train_dataset, test_dataset):
    """Create the training loop to load pretrained model and tokenizer and start the training process.
...
def run(args):
    """Load the data, train, evaluate, and export the model for serving and evaluating.
